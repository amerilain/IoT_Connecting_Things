...
void ssd1306::send_data(uint8_t* data, size_t length) {
    uint8_t control_byte = 0x40; // Control byte for data (Co = 0, D/C# = 1)
    std::vector<uint8_t> buffer;
    buffer.push_back(control_byte); // First byte is the control byte
    for (size_t i = 0; i < length; i++) {
        buffer.push_back(data[i]);
    }

    i2c_write_blocking(ssd1306_i2c, address, buffer.data(), buffer.size(), false);
}

void ssd1306::writeBuffer(uint8_t* buffer, size_t bufferSize) {
    send_data(buffer, bufferSize);
}